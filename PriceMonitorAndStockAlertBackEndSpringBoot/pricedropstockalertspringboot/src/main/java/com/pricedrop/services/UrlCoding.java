...
public static String extractProductName(String url) {
/*
    // Ensure the URL contains the expected parts
    if (url.contains("/dp/")) {
        // Split the URL at "/dp/"
        String[] parts = url.split("/dp/");
        if (parts.length > 0) {
            // The product name is in the preceding part before "/dp/"
            String productNamePart = parts[0]; // e.g., https://www.amazon.in/Samsung-Galaxy-Smartphone-Titanium-Storage
            // The product name is the last segment after the last "/" character
            String[] nameParts = productNamePart.split("/");
            String productName = nameParts[nameParts.length - 1]; // Extract the product name part
            return productName; // Return the extracted product name
        }
    }
